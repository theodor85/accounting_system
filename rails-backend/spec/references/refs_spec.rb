...
RSpec.describe 'Reference testing: ' do
  ...
  describe ::Metadata::References::Reference do
    it 'can create reference and find by name and id' do
      ref1 = ::Metadata::References::Reference.new
      ref1.ref_name = ref_name
      ...
      ref2 = ::Metadata::References::Reference.new
      ref2.find_by_name(ref_name)
  
      expect(ref1.ref_name).to             eq(ref2.ref_name)
      expect(ref1.ref_id).to               eq(ref2.ref_id)
      ...
      ref3 = ::Metadata::References::Reference.new
      ref3.find_by_id(ref_id)
  
      expect(ref1.ref_name).to             eq(ref3.ref_name)
      expect(ref1.ref_id).to               eq(ref3.ref_id)
      expect(ref1.fields[0]['name']).to    eq(ref3.fields[0]['name'])
      expect(ref1.fields[0]['type']).to    eq(ref3.fields[0]['type'])
      expect(ref1.fields[1]['name']).to    eq(ref3.fields[1]['name'])
      expect(ref1.fields[1]['type']).to    eq(ref3.fields[1]['type'])
      expect(ref1.fields[2]['name']).to    eq(ref3.fields[2]['name'])
      expect(ref1.fields[2]['type']).to    eq(ref3.fields[2]['type'])
    end

    it 'can delete reference' do
      ref_for_delete_name = 'test delete ref'
      
      ref1 = ::Metadata::References::Reference.new
      ref1.ref_name = ref_for_delete_name
      ref1.add_field(name: 'customer', type: 'string')
      ref1.create

      ref1.delete

      expect { ref1.find_by_name(ref_for_delete_name) }.to raise_error(::Metadata::References::GettingReferenceException)
    end
  end
